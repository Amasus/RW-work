...
Thirdly, in \cite{Wilsonloop}, the authors show that the matroid $M(\VP)$ is a positroid, i.e. the prametrized space $L(\VP)$ intersects the non-negative Grassmannian. Due to the parallels with the Amplituhedon, we are interested in the non-negative geometry of the spaces defined by the Wilson loop diagrams. In \cite{basisshapeloci}, the author shows that \ba \overline{\Lpos(\VP)} = \overline{\Sigmapos(\VP)} \label{eq:density}\;.\ea  In other words, up to a space of measure $0$, the non-negative space parametrized by the Wilson loop diagrams, $\{V_p\} \subset (\RP^{n})^k$ and $M_{\VP} \subset (\R^{n \times k})$ also parametrize positroid cells in the positive Grassmannians. 
...
For a positroid variety $\Sigma$, every boundary corresponds to setting a (set of) elements of the Grassman Necklace to 0. Indeed one postroid variety is said to be in the boundary of another if the set of non-vanishing Pl{\"u}cker coordinates of the first are contained in the second. However, this property does not hold when one considers the minimal representations of $\Sigma$. Specifically, if $M_\cV$ is a minimal representation of $\Sigma(\cV)$, then not every co-dimension one boundary of a $\Sigma(\cV)$ contains the vanishing locus of a factor of $R(\cV)$. \todo{Is this correct?}
...
\begin{rmk}
We remark that this cancelation is exact only in the space parametrized by matrices of the form $M(\VP)$, as a subspace of $\Gr(k,n)$ and not in the space parametrized by matrices of the form $M(\YP)$, as a subspace of $\Grall(k,n+1)$. In \cite{HeslopStewart,non-orientable}, the authors explicitly show that cancelations of this form do not hold in the larger space because of orientation issues. \todo{this makes me nervous. Does the cancelation actually happen on $\Gr(k,n)$ or $\Grall(k,n)$. I know things become sloppy as we leave the postive domain, but I don't have an understanding of how, other than smaller dimensional matroids sneak in.}
\end{rmk}
...
Let $(\cP, [n])$ be a diagram with propagators two propagators $p = (i, j)$ and $q = (i, k)$ adjascent on the edge $i$. After the reparametrization define in Lemma \ref{lem:simplifyR(W)}, setting $w$ to zero reduces the dimension by $1$ \hlfix{if and only if}{what if there is an $(i,l)$ with $j < l < k$? or does the word adjacent imply that?} there is not a propagator $r = (j, k)$ in $\cP$.
\end{cor}
\begin{proof}
For a Wilson loop diagram $(\cP, [n])$, let $\bf{x}$ be the parametrization that includes the variable $w$, and let $\bf{y}$ be the other set of variables. Note that the Jacobian for the change of variables given in Lemma \ref{lem:simplifyR(W)} has zero determinant when $x$ is sent to $0$. When $w$ are set to zero, \hlfix{the determinant remains non-zero. }{do you mean $y$ instead of $w$? setting $w = $ makes that determinant vanish.} Without loss of generality, assume that $w$ is a variable in the row corresponding to the propagator $p$.
...
First we show that when $w = 0$, the dimension of the span of a row set does not increase with the rows included in the set. In particular, we may write $x_{q, i} = \lambda x_{p, i}$ and $x_{q, i+1} = \lambda x_{p, i+1}$, with $\lambda$ a real variable. By adding a scalar multiple of $M'_p$ to $M'_q$, one gets a row that has independent variable entries exactly in the columns $V_r =\{j, j+1, k, k+1\}$. If $r$ is in $\cP$, the span of the rows $p$ and $q$ contains the span of the row $r$ in $\R^n$:
\ba \mathrm{span}(M'_{p,q}) =  \mathrm{span}(M'_{p,q,r})\;.\label{eq:spanfact}\ea
Thus, Theorem \ref{res:Rado} implies this boundary does not have codimension 1

Next, suppose $r \not \in \cP$, but there is a set of propagators $P \subset \cP$ such that $p \not \in P$ and  $\mathrm{span}(M'_{P \cup p}) = \mathrm{span}(M'_P)$ \hlfix{}{Is $p$ the row containing $w$? If so, I'd make a note saying all subsets of rows not containing $p$ satisfy hypotheses.}. Without loss of generality, assume that $P$ one of the smallest sets of propagators such that this is true. Note that $P$ cannot exist if $q \not \in \cP$, since all the variables in $M'_p$ and $M'_q$ are algebraically independent from the variables in $M'_{\cP\setminus \{p,q\}}$\hlfix{.}{oh. or if $p$ is the other row. in that case i'd like a note that including $q$ but not $p$ is indistinguishable from doing so before setting $w =0$. thus, counter examples need both.} 

Therefore, without loss of generality, assume $q \in P$. Since $\mathrm{span}(M'_{p}) \neq \mathrm{span}(M'_{q})$, the equality $\mathrm{span}(M'_{P \cup p}) = \mathrm{span}(M'_P)$ implies that  $\mathrm{span}(M'_{p\cup q}) \subseteq \mathrm{span}(M'_P)$. But equation \eqref{eq:spanfact} \hlfix{means that}{I don't see how (9) implies that. Is (9) for getting the contradiction in the next sentence?}
\ba \mathrm{span}(M'_{r})  \subseteq \mathrm{span}(M'_{P\setminus q}) \label{eq:spandfact2}\;.\ea
However, this cannot be true. We may write \hlfix{$M'_{P\setminus q} = M_{\cV(P\setminus q)}$}{I don't know what this notation means} as the set of propagators $P \setminus q$ is unaffected by the limits defining the matrix $M'$. By Theorem \ref{res:minimalrep}, $\dim(M_{\cV((P\setminus q) \cup r)}) = \dim(M_{\cV(P\setminus q)}) + 3$, which contradicts \eqref{eq:spandfact2}. Therefore, if $r \not \in \cP$, condition (i) of Theorem \ref{res:Rado} cannot holds, and thus the limit $w \rightarrow 0$ defines a codimension $1$ subspace of $L(M_{\VP})$.
\end{proof}
