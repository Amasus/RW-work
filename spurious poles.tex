...
\begin{prop}
Let $\Sigma$ be a positroid cell of dimension $d$, and $C$ be a variable valued matrix realizing $\Sigma$ with $d$ independent variables. Let $V$ and $W$ be \hlfix{cyclic flats}{needs definition} of less than full rank such that one is not contained in the other ($W \subsetneq V$) in the associated matroid $M(W)$. Suppose that $|\Prop(V) \setminus \Prop(W)| \geq |\Prop(W) \setminus \Prop(V)|$. A matroid defined by:
\begin{itemize}
\item Taking the entries of $C|_{W}$ and moving them to a row from $\Prop(V) \setminus \Prop(W)$ (each row in $\Prop(W) \setminus \Prop(V)$ goes to a row in $\Prop(V) \setminus \Prop(W)$ leaving all the columns the same, overlapping rows may be changed or not),
\item then zeroing out the corresponding elements in $\Prop(W) \setminus \Prop(V)$.
\end{itemize}
