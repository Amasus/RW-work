...
\subsubsection{Wilson loop diagrams as matrices \label{sec:WLDmatrix}}
For each propagator $p \subset \cP$ in admissible Wilson loop diagram, $W = (\cP, [n])$, associate a $4$ dimensional subspace of $\RP^{n+1}$ of the form \bas Y_p = \begin{cases}  x_{p,i} &  i = 0 \textrm{ or } i \in V_p \\ 0 &  \textrm{else,}\end{cases} \eas where $x_{p,i}$ are real valued variables. Then one may associated to $W$ a subspace of $\Grall(k, n+1)$ parametrized by the $Y_p$. That is, given the Wilson loop diagram $W$ above, with $p = (1,7)$, $q = (3,5)$ and $r = (2,5)$, one may write \bas x_*(W) = \begin{bmatrix} x_{p,0} & x_{p,1} & x_{p,2} & 0 &0 &0 &0 &x_{p,7} & x_{p,8} \\  x_{q,0} & 0 & 0 &x_{q,3} & x_{q,4} &  x_{q,5} & x_{q,6} & 0 & 0 \\ x_{r,0} & 0 & x_{r,2} & x_{r,3} & 0  &x_{r,5} & x_{r,6} & 0 &0\end{bmatrix}\eas \sanote{make certain all $c$s are changed to $x$}

We define the matrix $C(W)$ to be the one defined by ignoring the first column of $x_*(W)$.  In the running example, \bas C(W) = \begin{bmatrix}  x_{p,1} & x_{p,2} & 0 &0 &0 &0 &x_{p,7} & x_{p,8} \\    0 & 0 &x_{q,3} & x_{q,4} &  x_{q,5} & x_{q,6} & 0 & 0 \\  0 & x_{r,2} & x_{r,3} & 0  &x_{r,5} & x_{r,6} & 0 &0\end{bmatrix}\eas

\begin{rmk}
Note that, as a subspace of $\Grall(k, n)$ (resp. $\Grall(k, n+1)$), or ordering of the rows in $C(W)$, resp. $x_*(W)$ do not matter.
\end{rmk}

In this paper, we refer to the matrices $C(W)$ and $x_*(W)$ as matrices with algebraically independent invertible variables. In order to study matrices with these properties, we introduce some notation.
...
\begin{rmk}\label{rmk:C(W)notation}In the special case of a Wilson loop diagrams, for $W = (\cP, [n])$, we denoe $C(W) = M_{\VP}$ where $\VP = \{V_p | p \in \cP\}$ and  $C(W) = M_{\VPs}$ with $\VPs = \{x_{p,0} \cup V_p | p \in \cP\}$. \end{rmk}

\subsubsection{Wilson loop diagrams as matriods \label{sec:WLDmatroid}}

Before relating Wilson loop diagrams to matroids and positroid, we begin with a brief summary of the matroidal properties of Wilson loop diagrams, we give a brief overview of matroids and positroids.

A matroid can be defined as a set, and a set of independence conditions on said set. For instance, one can defined $M = (E, \mathcal{B})$, where $\mathcal{B}$, called a basis set, is a non-empty set of subsets of $E$, each of the same size, satisfying the basis exchange condition: \bas \textrm{ if }  a \in A\setminus B \textrm{ then } \exists b \in B \setminus A \textrm{ and } (A \setminus a) \cup b \in  \mathcal{B} \;.\eas Each element of $\mathcal{B}$ is a basis of $M$ and denotes a maximal independent set. The rank of the matroid, denoted $\rk(M)$ is the unique size of all the basis element. We may also refer tothe rank of a subset of $E$, $S \subset E$. We write the restriction of a matroid to $S$ $\rk(M|S) = \max \{B \subset S| B \in \mathcal{B} \}$, which, by abuse of notation, we also denote $\rk(S)$. 
...
\item Corrollary [3.39] shows that all admissible Wilson loop diagrams correspond to positroids. This means that, as matroids, the Wilson loop diagrams can be represented by matrices with all positive maximal minors. That is, the subspace of $\Grall(k,n)$ parametrized by $C(W)$ intersects $\Gr(k,n)$.
\end{enumerate}


Following the conventions of \cite{casestudy, generalcombinatorics1}, we call the positroid cell defined by the Wilson loop diagram $W$, $\Sigma(W)$. Indicate by $G(W)$ be the subspace of $\Gr(k,n)$ parametrized by $C(W)$ and let $\cW_{k,n}$ be the set of all admissible Wilson loop diagrams with $k$ propagators and $n$ vertices. In \cite[Theorem 8.4]{basisshapeloci}, the author writes $M_{\VPs}$ in lieu of $C(W)$ (according to Remark \ref{rmk:C(W)notation}) and shows that $ \overline{G(W)} = \overline{\Sigma(W)}$. In other words, the space $\cup_{W \in \cW_{k,n}}G(W)$ differs from the positroid cell $\Sigma(W)$ only on a set of measure 0. For an explict example of where these spaces differ, see Example \ref{eg:closuresmatch}. The author of \cite{basisshapeloci} also shows each $\Sigma(W)$ is a $3k$ dimensional cell.

In \cite[section 2.3]{non-orientable}, the authors show that  the subspace of $\Grall(k,n)$ parameterized by $x_*(W)$ can be viewed as a real $k$ bundle over the space parametrized by $C(W)$ . Restricting to $G(W)$ gives the vector bundle $G_*(W) \rightarrow G(W)$. As shown in Theorem 2.29 of loc. cit., the space $\cup_{W \in \cW_{k,n}}G_*(W)$ need not be orientable. 

We have now constructed a map from Wilson loop diagrams to the positroid cells. Let $\cW_{k,n}$ be the set of all admissible Wilson loop diagrams with $k$ propagators and $n$ vertices. Then \ba \cW_{k,n} \leftrightarrow (\RP^4 )^{\times k} \hookrightarrow (\RP^{n+1} )^{\times k} \leftarrow \cup_{W \in \cW_{k,n}}G_*(W)  \;. \label{eq:maps}\ea The composition of the first two maps comes from the definition of the vectors $Y_p$.  The map $\cup_{W \in \cW_{k,n}}G_*(W) \rightarrow (\RP^{n+1} )^{\times k}$ is formed by setting the first entry of each $Y_p$ to 1. (Note, that in making this choice, we are deliberately preventing the variables in the first column of $x_*(W)$ from being $0$. However, this is okay as those values correspond to the physical poles of the scattering amplitude, and therefore are not the subject of this paper.) Under this choice, it is a dense inclusion for each $W \in \cW_{k,n}$. In particular, this map misses exactly the points in the image of $W$ in $(\RP^{n+1} )^{\times k}$ where the set of vectors $Y_p$ would not have full rank when evaluated. However, this is a space of measure $0$ in the image of $W$ in $(\RP^{n+1} )^{\times k}$. 
...
\begin{dfn} \label{dfn:I(W)} \bas \cI(W) (\cZ_*)  = \int_{(\RP^4)^k} \frac{\prod_{p \in \cP} \prod_{v \in V_p} dx_{p, v}}{R(W)} \delta^{4k|4k}(x_*(W) \cdot \cZ_*) \eas where, for $X$ a $k \times n+4$ matrix, \bas \delta^{4k|4k}(X) = \prod_{b =1}^k (X_{b, 4+b})^4\delta^4((X_{b,1},X_{b,2},X_{b,3},X_{b,4}))  \eas and $R(W)$ is a polynomial determined from the $W$ as follows: 
\begin{enumerate}
...
$x_*(W)$ is orientable, the union of said spaces are not. Therefore,
...
be $\pm 1$, as the physical poles of $N=4 SYM$ occur when $x_{p,0} =
...
{\color{red} Do we have a proof that the space parametrized by the $x_*(W)$, or tiled by the $\Sigma(W)$, is orientable? Can we create such a proof using arguments similar to that showing that the full space is non-orientable? maybe skip the volume argument in this paragraph altogether?}
...
Let $\Sigma$ be a positroid cell of dimension $d$ in $\Gr(k,n)$, and $M_\cV$ be a variable valued matrix realizing $\Sigma$ with a minimal number of independent variables. Let $M(\cV)$ be the associated matroid. Let $V$ and $W$ be cyclic flats of less than full rank ($\rk (V), \; \rk(W) <k$) such that: \begin{enumerate} \item  $\Cols(V)$, $\Cols(W)$ and $\Cols(V \cup W)$ are cyclic intervals and \item one cyclc flat is not contained in the other (E.g. $W \subsetneq V$) \end{enumerate}  Without loss of generality, suppose that $|\Rows(V) \setminus \Rows(W)| \geq |\Rows(W) \setminus \Rows(V)|$. Let $C'$ be a variable valued matrix derived from $C$ defined by taking the non-zero entries of $C|_{W}$ and moving them to a 0 entry in $\Rows(V)$ and $\Cols(V\cup W)$ and then zeroing out the original elements in $C|_{W}$ such that circuits in $V$ and $W$ are preserved. If this process does not change the rank of $C$, then the Grassmann Necklace defined by $C'$ defines a positroid that lies in the boundary of the positroid $\Sigma$. \end{prop}

\begin{proof} 

If $GN(C')$ defines a positroid cell, $\Sigma'$ that is in the boundary of $\Sigma$, then the two Grassmann Necklaces, must be different, and the basis set of $C$ must contain the basis set of $C'$. 

First, we compare the Grassmann necklaces defining $\Sigma$ and $\Sigma'$. We may read these directly off the matrices $C$ and $C'$. Call them $GN(C)$ and $GN(C')$ respectively. In particular, we with to show that that $GN(C) \neq GN(C')$. To see this, first note that $\rk(V)$ (resp. $\rk(W)$) $>0$. If this were not true, then whichever flat had rank $0$ would be contained in the other flat, which contradicts our hypothesis. Since $V \cup W$ is a cyclic interval (in both $C$ and $C'$), let $v$ denote the first element of $\Cols(V\cup W)$ in the $>i$ ordering such that $\rk(v) >0$. Let $I_v$ and $I_v'$ be the Grassmann Necklace element starting at the column $v$ in $GN(C)$ and $GN(C')$ respectively. We have that $v \in I_v$ and \hlfix{$v \in I_v'$}{changed from $a$ to $v$}. 
...
Suppose, for contradiction, we have that $GN(C) = GN(C')$. Then we would have $I_v = I'_v$. But this would imply that $\rk(V \cup W)$ in $C'$ is $r+ s$. However, by construction, in $C'$, $\rk(V) = \rk(V\cup W)$, i.e. $r = r+s$ which contradicts the fact that $s > 0$. 
...
To see that basis set $\cB$ of $M(C)$ contains the basis set $\cB'$ of $M(C')$, note that any $B \in \cB$ that that does not intersect $V \cup W$ is also a basis set of $\cB'$ and vice versa. \st{Any basis set $B \in \cB$ intersecting $W$ is not a basis set in $\cB'$.} Let $B' \in \cB'$ be a basis set of $M(C')$ intersecting $V\cup W$. Partition $B'$ into two sets, those elements in $V \cup W$  and those not: $B' = B'_1 \cup B'_2$ with $B'_1 \subset V \cup W$ and $B'_2 \cap (V \cup W) = \emptyset$. If $B'$ were not a basis set of $M(C)$ ($B' \not \in \cB$), then $B'_1$ contains a circuit in $C$. But this would imply that a set of columns that formed a circuit in $C|_V$ or $C|_W$ became an independent set in $C'$ which violates the construction.
\end{proof}
