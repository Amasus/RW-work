...
\begin{proof}
This follows from the fact that $R^{\ast}(W)$ and by $R(W)$ are both radical polynomials defining the variety $\overline{\Sigma(W)}$, $R^{\ast}(W) = R(W)$. 

Theorem ?? from \cite{generalcombinatorics2} shows that $R(W)$ generates the radical of the ideal generated by $\prod_\mathcal{I} \Delta_{I_i}$, where $\mathcal{I}$ is the Grassmann Necklace of $\Sigma(W)$. Since the $\Delta_{I_i}$ do not vanish on $\Sigma(W)$, $R(W)$ vanishes on $\overline{\Sigma(W)} \setminus \Sigma(W)$. Therefore, it is the polynomial that generates the radical ideal of the variety $\overline{\Sigma(W)}\setminus \Sigma(W)$ \cite{??}

Since $R^*(W)$ is the product of $\Delta_{I^*_i}$, it too is the polynomial generating the  the radical ideal of the variety $\overline{\Sigma(W)}\setminus \Sigma(W)$. Therefore, $R^{\ast}(W) = R(W)$. \todo{Okay Cam, fix the hash I've made of this; Recall note sent to me on May 18}
\end{proof}
