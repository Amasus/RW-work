...
Suppose that the result holds for $k = l-1$. Let $M'$ be an $l \times n$ matrix such that $\mathrm{span}(M'_{\{1,\dots,l\}}) = \mathrm{span}(M'_{1,\dots,l-1})$, but that $\mathrm{span}(M'_I) \neq \mathrm{span}(M'_{I \cup j})$ for $j \notin I$ whenever \hlfix{$|I| < l$}{Accepted?}. That is, condition (i) fails to hold only for the sets $|I| = l-1$. %\st{For any subset $I \subset [k]$, let $d_I$ be the number of parameters in the $\mathbf{x}$ variables appearing in the rows of $M'$ indexed by $I$.}\sanote{nix. Not used. $d_l$ means something else below}

\st{Note that a plane in $L(M')$ is spanned by an element $r \in  \mathrm{span}(M'_{l})$, plus a $l-1$ plane in $L(M'_{1,\dots,l-1}) \cap \Grall(\mathbb{R}^n \perp r, k-1)$.} \todo{can we talk about planes in $\Grall$? - I switched the language to 'a plane in L(M'). I think this is clear since L(M) is a set of k-planes, but could add something else.}\todo{We talk about a span of a vector in $\R^n$ and a subspace of $\Grall$. That can't be right?! - This is what I want to say, but could change language to clear things up. Taking the span of a set of vectors is well defined. So, I take the set of vectors from a (k-1)-plane union one additional vector, and this span is a k-plane in L(M). I think the confusion here is that I am thinking of the grassmannian more set theoretically for this proof, "set of planes define in a certain way". So, when I talk about a k-1 plane, i am talking about an element of that setl} {\color{red} Define the rows of $M'$ such that $M'_{I\cup j}= M'$. Note that a point in $L(M')$ represeants an $l$ plane in $\R^n$ that is spanned by an element $r \in \textrm{span}(M'_j)$, plus an $l-1$ plane in $\R^n \perp \vec{r}$. }Since condition (i) does not hold for $I = \{1,\dots,l-1\}$, $r \in \mathrm{span}(M'_{1, \dots, l-1})$,
% 
