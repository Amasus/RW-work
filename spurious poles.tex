...
Thirdly, in \cite{Wilsonloop}, the authors show that the matroid $M(\VP)$ is a positroid, i.e. it $L(\VP)$ intersects the non-negative Grassmannian. Due to the parallels with the Amplituhedon, we are interested in the non-negative geometry of the spaces defined by the Wilson loop diagrams. In \cite{basisshapeloci}, the author shows that \ba \overline{\Lpos(\VP)} = \overline{\Sigmapos(\VP)} \label{eq:density}\;.\ea  In other words, up to a space of measure $0$, the non-negative space parametrized by the Wilson loop diagrams, $\{V_p\} \subset (\RP^{n})^k$ and $M_{\VP} \subset (\R^{n \times k})$ also parametrize positroid cells in the positive Grassmannians. 
...
We conclude this section with a brief aside, noting that the polynomial $\prod_{I_i \in \cI}I_i$ has appeared in relation to a cluster algebra associated to the positroid $\Sigma$ where $\cI= \{I_1,I_2, \dots, I_k\}$ be the associated Grassmann necklace \cite{LamGalashin, SS-BW}. Note that there is a difference in the Grassmann Necklace presented in loc. cit. and this work. Namely, in  in that work, the Grassmann necklace is a written in terms of the Pl\''ucker coordinates coming from $\Grall(k,n)$.  In this paper, we fix a (minimal) representaion, $M_{\cV}$, of $\Sigma(\cV)$. Then, instead of the Pl\''ucker coordinates $I_i$, we consider the minors $\Delta_{I_i}$ of $M_{\cV}$ as polynomials in $x_{p, i}$. 

In particular, in \cite{LamGalashin}, the authors consider $\cI^*$ to be the reverse Grassmann necklace associated to $\Sigma$. That is, $I^{\ast}_j$ is that maximal set in the $j^{th}$ cyclic shift of the Gale order on sets such that the Pl\"ucker coordinate $\Delta_{I^*_j}$ is non-vanishing on $\Sigma(\cV)$. Following Chapter 5 of \cite{knutsonlamspeyerjuggling} replacing Schubert varieties in the Grassmannian with reverse Schubert varieties, one sees $\Sigma$ can equivalently be defined as an intersection of reverse Schubert varieties. In \cite{SS_BW}, the authors show that each positroid variety defines a cluster algebra, and that the prime factors of $\prod_{I_i^\ast \in \cI^\ast}I_i^\ast$ are exactly the frozen variable of said cluster algebra. 
...
Fixing a set of subsets $\cV$, from Theorem 5.1 in \cite{basisshapeloc}, we know that $L(\cV) \subset \overline{\Sigma(\cV)}$. Let $V$ be the subset of $L(\cV)$ that is the image of the vanishing loci of $R(\cV)$ under the map in \eqref{eq:maps}. That is, $V = L(\cV) \cap \{\Delta_{I_i}(\mathbf{x}) = 0 | I_i \in \mathcal{I}\}$. Then,
...
\begin{thm}\label{res:Rado} {\color{red}SA: don't we need one of these to be a minimal parametrization? \\  CM: I actually don't think so. In fact, I think this theorem is strictly more general in the sense that if your variable transformation is just the identity, the condition from this theorem forces the minimality condition. \\ SA: But then how do we make the statement about the dimension being the number of variables left minus $k$ if there isn't a minimal parametrization? \\ CM: Take a parameterization that isn't minimal. Look at row operation you need to do to kill off a variable. For a minimal counterexample, taking any row on the right hand side to be $j$ and all the other rows to be $I$ will be a violation of (i). In that theorem statement, the minimality aspect is almost definitional: the subspace is parametrizable by definition, and if you have a parametrization with the same number of variables as the dimension, it must be minimal. That aspect of the theorem statement was mostly included because my argument at the time of proving it was a variable elimination thing and it seemed a good way to organize.} \\
...
\begin{itemize}
\item[(i)] For all $I \subsetneq [k]$ and all $j \in I^c$, $\mathrm{span}(M_I) \subsetneq \mathrm{span}(M_{I \cup j})$. That is, adding a row to $M_I$ always increases the size of the span.
...
\begin{displaymath}
L(M'_{1,\dots,l-1}) \cap \Grall(\mathbb{R}^n \perp r, k-1).
\end{displaymath}
Note that here we are talking about the Grassmannian set theoretically. So, since $\Grall(\mathbb{R}^n,k)$ is the set of $k$-planes in $\mathbb{R}^n$ and $(\mathbb{R}^n \perp r)$ is a subset of $\mathbb{R}^n$,
\begin{displaymath}
\Grall(\mathbb{R}^n \perp r,k) \subset \Grall(\mathbb{R}^n,k).
\end{displaymath}
\noindent
Thus, when we say $L(M'_{1,\dots,l-1}) \cap \Grall(\mathbb{R}^n \perp r, k-1)$, we mean the set of $(k-1)$-planes in $L(M')$ which are orthogonal to $r$.
...
Since condition (i) does not hold for $I = \{1,\dots,l-1\}$, $r \in \mathrm{span}(M'_{1, \dots, l-1})$. \hlfix{So, the set of planes in $L(M'_{1,\dots,l-1})$ containing $r$ has positive dimension aside from in the trivial case when $L(M'_{1,\dots,l-1}) = \mathrm{span}(r)$.}{This doesn't really feel necessary, but maybe it helps?} Thus,
% 
\begin{displaymath}
