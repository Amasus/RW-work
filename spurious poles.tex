...
\begin{thm}
Let $W=(\mathcal{P},[n])$ be an admissible Wilson loop diagram and let $G' \subset G(W)$ be the vanishing locus of a single factor of $R(W)$ inside $G(W)$ and suppose that $G'$ has codimension $1$ in $G(W)$. Let the open positroid $\Sigma'$ be the boundary of $\Sigma(W)$ corresponding to $G'$. Then, $G'$ is dense in $\Sigma'$.
\end{thm}
