...
Note that in this definition, we take the integral over $(\RP^4)^k$, while previously, we have referred to the matrices as parametrizing subspaces of $\Gr(k,n)$. This is permissible due to the density arguments following display \eqref{eq:maps}. \sanote{Cameron, I definitely need a second brian on this paragraph.}

The physical poles of the tree level amplitude $A_{k,n}$ arise when the variables $x_{p, 0} = 0$. The spurious poles occur when $R(W)$ evaluates to $0$ \cite{??}. In this paper, we are interested only in the degree one poles of $R(W)$. Namely, Theorem \ref{res:deg1polescancel} shows that the all the degree 1 poles in the sum  $A_{k,n}$ cancel.
...
\begin{thm}
\hlfix{Let $W=(\mathcal{P},[n])$ be an admissible Wilson loop diagram and let $L' \subset L(\VP)$ be the vanishing locus of a single factor of $R(W)$ inside $L(\VP)$ and suppose that $L'$ has codimension $1$ in $L(\VP)$. Let the open positroid $\Sigma'$ be the boundary of $\Sigma(W)$ corresponding to $L'$. Then, $L'$ is dense in $\Sigma'$.}{note, I changed your notation here, but not in your proof}
\end{thm}
...
Note that not all factors of $R(W)$ correspond to codimension 1 poles. 
