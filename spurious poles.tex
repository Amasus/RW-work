...
Let $G(W)$ be the geometric space parameterized by the Wilson Loop diagram $W$. From Theorem ?? in \cite{basisshapeloc}, $G(W) \subset \overline{\Sigma(W)}$. Let $V$ be the subset of $G(W)$ where $R(W)$ vanishes. Then, \todo{Once the map is from $n \times k$ matrices to Wilson loop cell is defined, should probably clarify things are fibers over that, $\pi^{-1}(\Sigma(W))$ rather than just $\Sigma(W)$ for instance.}
%
