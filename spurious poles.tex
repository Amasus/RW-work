...
Let $(\cP, [n])$ be a diagrapm with propagators $p = (i, j)$ and $q = (i, k)$. After the reparametrization define in Lemma \ref{lem:simplifyR(W)}, setting $w$ to zero reduces the dimension by $1$ if and only if there is not a propagator $r = (j, k)$ in $\cP$.
\end{cor}
\begin{proof}
...
Therefore, when $w = 0$, the dimension of the span of a row set increases with the rows included in the set. If $r$ is in $\cP$, then, $\mathrm{span}(M'_{p, q, r}) = \mathrm{span}(M'_{q, r})$. Therefore, by Theorem \ref{res:Rado}, this boundary does not have codimension 1.

If $r$ is not in $\cP$, then suppose there is a set of propagators $P \subset \cP$ such that $p \not \in P$ and  $\mathrm{span}(M'_{P \cup p}) = \mathrm{span}(M'_P)$. Note that this cannot be true if $q \not \in \cP$, since all the variables in $M'_p$ and $M'_q$ are algebraically independent from the variables in $M'_{\cP\setminus \{p,q\}}$. Therefore, assume $q \in P$.  Then we have that $\mathrm{span}(M'_{(P \setminus q) \cup p}) \supsetneq \mathrm{span}(M'_{P \setminus q})$ and that $\mathrm{span}(M'_{P}) \supsetneq \mathrm{span}(M'_{P \setminus q})$. \todo{finish this. Don't quite see the argument from here}



By Theorem \ref{res:minimalrep}, condition (i) holds for $M_{\VP}(\bf{y})$. Furthermore, since 
\end{proof}
