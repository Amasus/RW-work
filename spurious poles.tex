...
\begin{dfn}\hlfix{If the matroid $M(\cV)$, is a positroid, let $\Sigma(\cV)$ be the subspace of $\Grall(k,n)$ defined by the corresponding positroid variety}{This is anguished English}. We write $\Sigmapos(\cV) = \Sigma(\cV) \cap \Gr(k,n)$ to be the positroid cell defined when this is retricted to $\Gr(k,n)$. \end{dfn}
...
Secondly, note that in this definition, we take the integral over $(\RP^4)^k$, while the remainder of the paper referrs to the matrices as parametrizing subspaces of $\Gr(k,n)$. To see why this is possible, first note that every diagram $W = (\cP, [n])$  defines a map between products of projective spaces: \bas  \xymatrix{(\RP^4 )^{\times k}  \ar@{^{(}->}[r]^-{W} &  \{Y_p | p \in \cP\} \subset (\RP^{n+1} )^{\times k}  } \;. \eas When the set of vectors $\{Y_p | p \in \cP\}$ are in general position, they define a $k$-dimensional hyperplane in $\RP^{n+1}$, i.e. they parametrize $L(\YP)$. Namely, the integal $I(\VP)$ is defined on all but the set of measure 0 where $M_{\YP}$ is of less than full rank. This subspace is exactly $L(\YP)$, which differs from $\Sigma(\VP) \times \R^k$ by a set of measure 0. 


Thirdly, in \cite{wilsonloops}, the authors show that the matroid defined by $M_{\VP}$ is a positroid, i.e. it $L(\VP)$ intersects the non-negative Grassmannian. Due to the parallels with the Amplituhedon, we are interested in the non-negative geometry of the spaces defined by the Wilson loop diagrams. In \cite{basisshapeloci}, the author shows that \ba \overline{\Lpos(\VP)} = \overline{\Sigmapos(\VP)} \label{eq:density}\;.\ea  In other words, up to a space of measure $0$, the non-negative space parametrized by the Wilson loop diagrams, $\{V_p\} \subset (\RP^{n})^k$ and $M_{\VP} \subset (\R^{n \times k})$ also parametrize positroid cells in the positive Grassmannians. 
...
\begin{eg} \label{eg:differentpolys} Note that the polynomial $R(\cV)$ is dependent on the minimal parametrization of the postroid defined by the Grassmann Neclace $\cI$.  For instance, in \cite{GWLDCI}, the authors give conditions for when multiple Wilson loop diagrams can correspond to the same positroid cell. For example both $\cV_1 = \{\{1, 2, 4, 5\}, \; \{1, 2, 3, 4\} \}$ and $\cV_2 = \{\{1, 2, 4, 5\}, \; \{2, 3, 4, 5\} \}$ define the same matroid. However \bas R(\cV_1) = x_{1,2}(x_{1,1}x_{2,2}- x_{2,1}x_{1,2})x_{2,1}x_{2, 3}x_{2,4}x_{1,4}x_{1,5}  \quad \textrm{while} \\ R(\cV_2) =  x_{1,1}x_{1,2}x_{2,2}x_{2, 3} x_{2,5}(x_{1,4}x_{2,5}- x_{2,4}x_{1,5})x_{1,4}\;.\eas Both correspond to the polynomial formed by the product of the prime factors of $\prod_{i=1}^{n}\Delta_{I_i}$ under the corresponding coordinate system.
...
\note{$\Sigma$ here is for the full grassmannian}
...
We conclude this section with a brief aside, noting that the polynomial $R(\cV) = \prod_{I_i \in \cI}I_i$ has appeared in relation to a cluster algebra associated to the positroid $\Sigma(\cV)$ where $\mathcal{I} = \{I_1,I_2, \dots, I_k\}$ be the Grassmann necklace associated to $\Sigma(\cV)$. In particular, \hlfix{take a stab at summarizing the Lam Galashin and SS-BWwork}{do this}
...
\begin{thm}
Using the notation from above, $R^{\ast}(\VP) = R(\VP)$.
\end{thm}
{\color{red}\begin{enumerate}
\item In this section/ proof, are we talking about the full Grassmannian or the non-negative?
\item Does this theorem hold in general? I think it should. However, we also talk about pulling back to $(\RP^4)^k$, and I'm not certain why we are doing that here.
\end{enumerate}}
\begin{proof}
...
\begin{dfn}
For an $n \times k$ variable valued matrix $M$ and a set $I \subseteq [k]$, define $\mathrm{span}(M_I)$ to be the subset of $\mathbb{R}^n$ obtained by evaluating linear combinations of the rows $M$ indexed by $I$ at real parameters.
\end{dfn}
...
\item[(ii)] $\mathrm{dim}(G) = d-k$.\sanote{$G = M'$?}
\end{itemize}
\end{thm}
...
$G$ is spanned by an element $r$ in \hlfix{$G_{\{l\}}$}{???? what is this}, plus a $k-1$ plane in $G_{\{1,\dots,l-1\}} \cap \Gr(\mathbb{R}^n \perp r, k-1)$. Since $r \in R_{1, \dots, l-1}$,
% 
\sanote{stopped here}
\begin{displaymath}
