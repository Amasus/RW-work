...
Finally, we are ready to show that the codimension 1 singularies appearing in the integrals $I(\VP)$ cancel in the sum given in \eqref{eq:treelevelamplitude}. We do this by comparing positroid varieties with common codimension 1 boundaries that contain the vanishing loci of the polynomials $R(\VP)$. We first show that there are codimension 1 boundaries of a postitroid variety $\Sigma(\cV)$ that do not contain the vanishing loci of any factor of $R(\cV)$ with codimension 1. \hlfix{This phenomena occurs because while $L(\cV)$ is dense in $\overline{\Sigma(\cV)}$, $L(\cV)$ has empty intersection with certain boundary positroids. Since the vanishing set of $R(\VP)$ is a subset of $L(\cV)$, it will not contain these boundary positroids which do not intersect $L(\cV)$.}{maybe this is saying a little too much?}
