...
\begin{dfn}
Let $C$ be a \hlfix{variable valued matrix}{define this as the $M_\mathcal{S}$ as Cam does in Basis Shape Loci}. Denote by $r$ a row of $C$, and $c$ a column of $C$. Then we write $\Cols(r)$ to be the columns where the row $r$ has non-zero entries. Similarly, write $\Rows(c)$ to be the rows where the column $c$ has non-zero entries. 
\end{dfn}
...
\begin{rmk} \sanote{needs to be moved somewhere else}
...
Let $\Sigma$ be a positroid cell of dimension $d$ in $\Gr(k,n)$, and $C$ be a variable valued matrix realizing $\Sigma$ with $d + k$ independent variables, and $M(C)$ the associated matroid. Let $V$ and $W$ be cyclic flats of less than full rank ($\rk (V), \; \rk(W) <k$) such that: \begin{enumerate} \item  $\Cols(V)$, $\Cols(W)$ and $\Cols(V \cup W)$ are cyclic intervals and \item one cyclc flat is not contained in the other ($W \subsetneq V$) \end{enumerate}  Without loss of generality, suppose that $|\Rows(V) \setminus \Rows(W)| \geq |\Rows(W) \setminus \Rows(V)|$. Let $C'$ be a variable valued matrix derived from $C$ defined by taking the non-zero entries of $C|_{W}$ and \hlfix{moving them to a 0 entry in $\Rows(V)$ and $\Cols(V\cup W)$}{have to move it to the same column, right?} and then zeroing out the original elements in $C|_{W}$ such that circuits in $V$ and $W$ are preserved. If this process does not change the rank of $C$, then the Grassmann Necklace defined by $C'$ defines a positroid that lies in the boundary of the positroid $\Sigma$. \end{prop}
...
\begin{thm}
Let $\mathcal{P}$ be a collection of subsets, and let $\mathbf{x} = x_{i,j}$ and $\mathbf{y} = y_{i,j}$ be two sets of algebraically independent invertible variable which are related by a change of variable matrix with nonzero Jacobian. Let $M_{\mathcal{P}}(\mathbf{x})$ and $M_{\mathcal{P}}(\mathbf{y})$ be the variable valued matrices associated to $\mathcal{P}$ in the variables $\mathbf{x}$ and $\mathbf{y}$. Let $S \subset \mathbf{x}$ and consider $M' = M_{\mathcal{P}}(\mathbf{y})|_{x_{i,j} = 0; x_{i,j} \in S}$. Suppose there are $d$ variables remaining. Let $G$ be the subset of $\Gr(k,n)$ consisting of row spaces of matrices obtained by evaluating the remaining $x_{i,j}$ in $M'$ at real parameters. For $I \subset \{1, \dots, k\}$, let $R_I$ be the subset of $\mathbb{R}^n$ obtained by evaluating linear combinations of the rows of $M'$ indexed by $I$ at real parameters. The following are equivalent:
\begin{itemize}
